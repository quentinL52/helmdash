    ...
    <div className="min-h-screen bg-background text-foreground font-sans selection:bg-primary selection:text-primary-foreground" style={{ fontFamily: '"IBM Plex Sans", system-ui, sans-serif' }}>
      ...
      <header className="sticky top-0 z-50 bg-background/80 backdrop-blur-md border-b border-border">
        <div className="max-w-6xl mx-auto px-8 py-3.5 flex items-center gap-5">
          ...
          <nav className="flex gap-6 ml-6 text-[13px] font-medium text-muted-foreground" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>
          ...
          <div className="ml-auto flex items-center gap-4">
            <Link href="/auth" className="text-[13px] font-medium text-muted-foreground" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>{t('login')}</Link>
            <a href="#pricing" className="text-[13px] font-semibold bg-primary text-primary-foreground text-primary-foreground px-4 py-2 rounded-lg hover:-translate-y-px hover:shadow-lg transition-all" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>{t('pricingCtaPrimary')}</a>
          </div>
        </div>
      </header>
      ...
      <section id="top" className="max-w-6xl mx-auto px-8 pt-20 pb-10">
        <div className="inline-flex items-center gap-2 border border-border rounded-full py-1.5 pl-2 pr-3 mb-8">
          ...
          <span className="text-xs font-medium text-foreground tracking-wide" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>
        ...
        <div className="flex flex-wrap gap-3.5 items-center mb-4">
          <a href="#pricing" className="text-[15px] font-semibold bg-primary text-foreground px-6 py-4 rounded-lg hover:-translate-y-0.5 hover:shadow-xl hover:shadow-primary/30 transition-all" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>{t('ctaPrimary')}</a>
          <a href="#solution" className="text-[15px] font-semibold text-foreground px-5 py-4 rounded-lg border border-border hover:bg-muted transition-all" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>{t('ctaSecondary')}</a>
        </div>
        <p className="text-[12.5px] text-muted-foreground" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>{t('noCreditCard')}</p>

        <div className="flex items-center gap-3.5 mt-14 pt-5 border-t border-border">
          <span className="text-[11px] tracking-[1.5px] text-muted-foreground uppercase" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>{t('socialProof')}</span>
      ...
      <section id="probleme" className="max-w-6xl mx-auto px-8 py-16">
        <span className="text-xs tracking-[1.5px] text-primary font-semibold" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>{t('problemSurtitle')}</span>
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(250px,1fr))] gap-px bg-muted border border-border rounded-2xl overflow-hidden">
          {t.raw('pains').map((p: any) => (
            <div key={p.tag} className="bg-muted p-7 min-h-[210px] flex flex-col hover:bg-muted/80 transition-colors">
              <div className="text-[13px] font-bold text-primary mb-5" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>{p.tag}</div>
      ...
      <section id="solution" className="bg-primary text-primary-foreground text-primary-foreground py-20">
        <div className="max-w-6xl mx-auto px-8">
          <span className="text-xs tracking-[1.5px] text-primary font-semibold" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>{t('productSurtitle')}</span>
          ...
          <div className="grid grid-cols-[repeat(auto-fit,minmax(220px,1fr))] gap-6 mt-10">
            {t.raw('productFeatures').map((f: any, i: number) => (
              <div key={i}>
                <div className="text-xs font-bold text-primary mb-2.5" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>→</div>
                <h3 className="text-[17px] font-semibold text-primary-foreground mb-1.5 tracking-[-0.4px]" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>{f.title}</h3>
      ...
      <section className="max-w-6xl mx-auto px-8 py-20">
        <span className="text-xs tracking-[1.5px] text-primary font-semibold" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>{t('transparencySurtitle')}</span>
      ...
      <section id="pricing" className="bg-muted border-y border-border py-20">
        <div className="max-w-6xl mx-auto px-8">
          <span className="text-xs tracking-[1.5px] text-primary font-semibold" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>{t('pricingSurtitle')}</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 max-w-3xl">
            <div className="bg-primary text-primary-foreground text-primary-foreground border-[1.5px] border-primary rounded-2xl p-7 relative">
              <div className="flex items-center gap-2 mb-1.5">
                <h3 className="text-base font-semibold tracking-[-0.3px]" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>{t('cohort', { name: currentCohort.toUpperCase() })}</h3>
              </div>
              <div className="flex items-baseline gap-1.5 mt-5 mb-1">
                ...
                <span className="text-sm text-muted-foreground" style={{ fontFamily: '"IBM Plex Mono", monospace' }}> {t('perMonth')}</span>
              </div>
              <div className="text-[11.5px] text-muted-foreground mb-5" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>{t('billedSemiannually', { amount: COHORT_CONFIG[currentCohort].prices.semi_annual?.amount ? (COHORT_CONFIG[currentCohort].prices.semi_annual.amount / 100).toFixed(0) : 0 })}</div>
              <a href="/auth" className="mt-8 block text-center text-sm font-semibold py-3 rounded-lg bg-primary text-foreground transition-opacity hover:opacity-90" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>{t('pricingCtaPrimary')}</a>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="roadmap" className="max-w-6xl mx-auto px-8 py-20">
        <span className="text-xs tracking-[1.5px] text-primary font-semibold" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>{t('roadmapSurtitle')}</span>
          ...
          <ul className="space-y-4 max-w-xl">
            {t.raw('roadmapItems').map((item: any, i: number) => (
              <li key={i} className="flex justify-between items-center py-2 border-b border-border last:border-0">
                ...
                <span className="text-[11px] font-semibold px-2 py-1 bg-muted rounded text-muted-foreground uppercase" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>{item.status}</span>
              </li>
            ))}
          </ul>
        </div>
      ...
      <section className="bg-primary text-primary-foreground text-primary-foreground py-20">
        <div className="max-w-4xl mx-auto px-8 text-center">
          <span className="text-xs tracking-[1.5px] text-primary font-semibold" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>{t('makerSurtitle')}</span>
      ...
      {/* ===== FAQ ===== */}
      <section className="max-w-6xl mx-auto px-8 py-20">
        <span className="text-xs tracking-[1.5px] text-primary font-semibold" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>{t('faqSurtitle')}</span>
      ...
      <footer className="bg-card text-muted-foreground py-12">
        <div className="max-w-6xl mx-auto px-8">
          <div className="flex flex-wrap gap-10 justify-between items-start pb-9 border-b border-border">
            <div className="max-w-[300px]">
              <div className="flex items-center gap-2.5 mb-3.5">
                ...
                <span className="font-semibold text-[17px] text-primary-foreground tracking-[-0.5px]" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>Helmdash</span>
              </div>
          ...
          <div className="flex flex-wrap gap-3 justify-between items-center pt-5">
            <div className="text-xs text-muted-foreground" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>{t('footerCopyright')}</div>
            <div className="flex gap-5 text-xs text-muted-foreground" style={{ fontFamily: '"IBM Plex Mono", monospace' }}>
