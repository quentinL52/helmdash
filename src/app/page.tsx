...
export default function LandingPage() {
  ...
  const COMPARISON_ROWS = [
    { label: t('comparison.rows.0.label'), notion: t('comparison.rows.0.notion'), gpt: t('comparison.rows.0.gpt'), linear: t('comparison.rows.0.linear'), helm: t('comparison.rows.0.helm') },
    { label: t('comparison.rows.1.label'), notion: t('comparison.rows.1.notion'), gpt: t('comparison.rows.1.gpt'), linear: t('comparison.rows.1.linear'), helm: t('comparison.rows.1.helm') },
    { label: t('comparison.rows.2.label'), notion: t('comparison.rows.2.notion'), gpt: t('comparison.rows.2.gpt'), linear: t('comparison.rows.2.linear'), helm: t('comparison.rows.2.helm') },
    { label: t('comparison.rows.3.label'), notion: t('comparison.rows.3.notion'), gpt: t('comparison.rows.3.gpt'), linear: t('comparison.rows.3.linear'), helm: t('comparison.rows.3.helm') },
    { label: t('comparison.rows.4.label'), notion: t('comparison.rows.4.notion'), gpt: t('comparison.rows.4.gpt'), linear: t('comparison.rows.4.linear'), helm: t('comparison.rows.4.helm') },
    { label: t('comparison.rows.5.label'), notion: t('comparison.rows.5.notion'), gpt: t('comparison.rows.5.gpt'), linear: t('comparison.rows.5.linear'), helm: t('comparison.rows.5.helm') },
  ];
    ...
    <div style={{ background: "#E9E4D8", color: "#0E1B2E", fontFamily: '"IBM Plex Sans", system-ui, sans-serif', minHeight: '100vh', overflow: "hidden" }}>
      ...
      <section id="comparatif" style={{ background: "#F5F1E8", borderTop: "1px solid rgba(14,27,46,.1)", borderBottom: "1px solid rgba(14,27,46,.1)", padding: "76px 0" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 32px" }}>
          ...
          <div style={{ overflowX: "auto", border: "1px solid rgba(14,27,46,.14)", borderRadius: 14, background: "#EFEADD" }}>
            <div style={{ minWidth: 680 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr 1fr 1fr 1.1fr", background: "#E4DECF", borderBottom: "1px solid rgba(14,27,46,.14)" }}>
                <div style={{ fontFamily: '"IBM Plex Mono", monospace', padding: "16px 18px", fontSize: 12, letterSpacing: ".5px", color: "#6a6656", fontWeight: 600 }}>{t('comparison.col1')}</div>
                <div style={{ fontFamily: '"IBM Plex Mono", monospace', padding: "16px 12px", fontSize: 13, fontWeight: 600, textAlign: "center", color: "#3a4656" }}>Notion</div>
                <div style={{ fontFamily: '"IBM Plex Mono", monospace', padding: "16px 12px", fontSize: 13, fontWeight: 600, textAlign: "center", color: "#3a4656" }}>ChatGPT</div>
                <div style={{ fontFamily: '"IBM Plex Mono", monospace', padding: "16px 12px", fontSize: 13, fontWeight: 600, textAlign: "center", color: "#3a4656" }}>Linear</div>
                <div style={{ fontFamily: '"IBM Plex Mono", monospace', padding: "16px 12px", fontSize: 13, fontWeight: 700, textAlign: "center", color: "#F5F1E8", background: "#0E1B2E", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}><img src="/helmdash-mark-coral-512.png" width={14} height={14} alt="" />Helmdash</div>
              </div>
              {COMPARISON_ROWS.map((r) => (
                <div key={r.label} style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr 1fr 1fr 1.1fr", borderBottom: "1px solid rgba(14,27,46,.08)" }}>
                  <div style={{ padding: "15px 18px", fontSize: 14, color: "#2a3646", fontWeight: 500, display: "flex", alignItems: "center" }}>{r.label}</div>
                  <div style={{ fontFamily: '"IBM Plex Mono", monospace', padding: "15px 12px", fontSize: 12, textAlign: "center", color: "#7a7666", display: "flex", alignItems: "center", justifyContent: "center" }}>{r.notion}</div>
                  <div style={{ fontFamily: '"IBM Plex Mono", monospace', padding: "15px 12px", fontSize: 12, textAlign: "center", color: "#7a7666", display: "flex", alignItems: "center", justifyContent: "center" }}>{r.gpt}</div>
                  <div style={{ fontFamily: '"IBM Plex Mono", monospace', padding: "15px 12px", fontSize: 12, textAlign: "center", color: "#7a7666", display: "flex", alignItems: "center", justifyContent: "center" }}>{r.linear}</div>
                  <div style={{ fontFamily: '"IBM Plex Mono", monospace', padding: "15px 12px", fontSize: 12.5, fontWeight: 600, textAlign: "center", color: "#0E1B2E", background: "rgba(240,82,46,.07)", display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}><span style={{ color: accent, fontWeight: 700 }}>✓</span>{r.helm}</div>
                </div>
              ))}
            </div>
          </div>
          <p style={{ fontFamily: '"IBM Plex Mono", monospace', fontSize: 12, color: "#8a8474", margin: "16px 0 0" }}>{t('comparison.footer')}</p>
        </div>
      </section>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 20 }}>
          {PRICING_PLANS.map((pl) => (
            <div key={pl.name} className="hover:-translate-y-[3px] hover:shadow-[0_18px_40px_-18px_rgba(14,27,46,.4)] transition-all" style={{ background: pl.bg, color: pl.fg, border: `1.5px solid ${pl.border}`, borderRadius: 16, padding: 28, position: "relative", display: "flex", flexDirection: "column" }}>
              ...
              <a href="#" className="hover:opacity-90 transition-opacity" style={{ fontFamily: '"IBM Plex Mono", monospace', marginTop: "auto", textAlign: "center", fontSize: 14, fontWeight: 600, padding: 13, borderRadius: 10, background: pl.ctaBg, color: pl.ctaFg, border: `1px solid ${pl.ctaBorder}` }}>{pl.cta}</a>
            </div>
          ))}
        </div>
      </section>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 32px" }}>
              ...
              {[
                ...
                <div key={c.head}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
                    {c.links.map(l => (
                      <a key={l} href="#" className="hover:text-[#EAE6DC] transition-colors" style={{ fontSize: 13.5, color: "#9aa4b3" }}>{l}</a>
                    ))}
                  </div>
                </div>
              ))}
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 12, justifyContent: "space-between", alignItems: "center", paddingTop: 22 }}>
            ...
            <div style={{ fontFamily: '"IBM Plex Mono", monospace', display: "flex", gap: 20, fontSize: 12, color: "#5a6678" }}>
              <a href="#" className="hover:text-[#9aa4b3] transition-colors">{t('footer.legal.0')}</a>
              <a href="#" className="hover:text-[#9aa4b3] transition-colors">{t('footer.legal.1')}</a>
              <a href="#" className="hover:text-[#9aa4b3] transition-colors">{t('footer.legal.2')}</a>
            </div>
          </div>
        </div>
      </footer>
